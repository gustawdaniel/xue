...
describe('reverso', () => {
  it('works with mocked response', async () => {
    const scope = nock('https://api.reverso.net').post('/translate/v1/translation').reply(200, reversoResponse)

    scope.persist()

    const translations = await new Translator().translateWithProvider(
      'woda',
      'pl',
      'en',
      'reverso'
    )
    expect(translations[0].word).toEqual('water')
  })
})
