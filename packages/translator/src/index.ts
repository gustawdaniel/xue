...
import qs from "qs";
import { translations, Lang, TranslationProvider, PartOfSpeech } from "database";
...
export class Translator {
  ...
  private readonly cacheService: Pick<CacheService, "wrap"> = { wrap: CacheService.noWrap };
  ...
  private async google(word: string, source: Lang, target: Lang): Promise<Pick<translations, "to_text" | "meanings">> {
    ...
    const [res, _details] = await this.cacheService.wrap<[string, any]>(async () => {
      const client = new v2.Translate({
        credentials: this.googleConfig
      });

      return await client.translate(word, target);
    }, { type: "translation", word, source, target, provider: "google" }, ["google"]);
  ...
  private async deepl(word: string, source: Lang, target: Lang): Promise<Pick<translations, "to_text" | "meanings">> {
    ...
    // TODO: fix any
    const data = await this.cacheService.wrap<CorrectDeeplResponse>(async () => {
    ...
    }, { type: "translation", word, source, target, provider: "deepl" }, ["deepl"]);

    return {
      to_text: data.translations.map(t => t.text).find(() => true) ?? "",
      meanings: data.translations.map((trans: { detected_source_language: string; text: string }) => ({
        to_text: trans.text,
        pos: "other",
        sentences: []
      }))
  ...
  // }
  private async reverso(word: string, source: Lang, target: Lang): Promise<Pick<translations, "to_text" | "meanings">> {
    ...
    const data = await this.cacheService.wrap<CorrectReversoResponse>(async () => {
    ...
    }, { type: "translation", word, source, target, provider: "reverso" }, ["reverso"]);

    function posReverso(pos: string): PartOfSpeech {
      switch (pos) {
        case 'n.': return 'noun';
        default: {
    ...
    return {
      ...
      meanings: data.contextResults.results.map((res) => ({
        ...
        pos: posReverso(res.partOfSpeech),
        sentences: res.sourceExamples.map((s, i) => ({
          from_lang: source,
          from_text: s,
          to_lang: target,
          to_text: res.targetExamples[i]
        }))
      }))
    }
  ...
  async translateWithAllProviders(where: StrictWordsWhereInput): Promise<FullTranslationResponse[]> {
    ...
    return Promise.all(
      providers.map(async (provider): Promise<FullTranslationResponse> => {
        const translations = await this.translateWithProvider(where.word, where.source, where.target, provider);
        return {
          provider,
          translations
        };
  ...
  async translate(where: StrictWordsWhereInput): Promise<TranslationWitchProviders[]> {
