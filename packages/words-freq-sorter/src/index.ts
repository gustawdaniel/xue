...
export class TreeNode {
  ...
  static  inorder(root: null | TreeNode, arr: RangedWord[]) {
    if (root != null) {
      ...
      arr.push(...[...root.words.values()].map((word, index) => ({
        word,
        freq: root.freq,
  ...
  static  insert( node: TreeNode | null , freq: number, word: string) {
    if (node == null)
      return TreeNode.newNode(freq, word);
    ...
    if (freq < node.freq)
      node.left = TreeNode.insert(node.left, freq, word);
    else
      node.right = TreeNode.insert(node.right, freq, word);
