...
export const findRandomWord = protectedProcedure
  .input(z.object({ course_id: ObjectIdSchema, futureAnswers: z.array(z.number()).default([]) }))
  .query(async ({ input, ctx }) => {
    ...
    if (checked_iteration) {
      ...
      if (repetition) {
        const word = await prisma.words.findUnique({
          where: {
            id: repetition.word_id,
          },
        })

        if (word) {
          return { new: false, word: word.word }
        }
      }
    }
    ...
    if (!wordOrNull) throw new TRPCError({
