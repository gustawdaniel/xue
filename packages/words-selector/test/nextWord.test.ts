...
describe("I can see next words", () => {
  ...
  it("i never do not know answer", () => {
    ...
    expect(seq).toStrictEqual<number[]>(
      new Array(20).fill(0).map((e, i) => i + 1)
    );
  ...
  it("i always know the answer", () => {
    ...
    expect(wsSpeed.history_ranges.length).toBeGreaterThanOrEqual(
      wsSlow.history_ranges.length
    );
  ...
  it("i know all words until 100, then I do not know any word", () => {
    ...
    const conv = seq.reduce((p, n, i) => {
      const growthBelow100 =
        (n < 100 && dSeq[i] > 0) || (n > 100 && dSeq[i] < 0) ? 1 : -1;
  ...
  it("i know all words until 100, and some above 200", () => {
    ...
    const bot = new AnsweringBot(
      (r) => Number(r <= 100) + 0.2 * Number(r > 200)
    );
    ...
    expect(ws.knownList.filter((v) => v > 200).length).toBeGreaterThanOrEqual(
      0
    );
  ...
  it("I know half of words for any rango", () => {
    ...
    expect(Math.max(...out["0.1"].max.map((m) => m.y))).toBeLessThan(
      Math.max(...out["0.5"].max.map((m) => m.y))
    );
    expect(Math.max(...out["0.5"].max.map((m) => m.y))).toBeLessThan(
      Math.max(...out["0.9"].max.map((m) => m.y))
    );

    expect(Math.min(...out["0.1"].min.map((m) => m.y))).toBeLessThanOrEqual(
      Math.min(...out["0.5"].min.map((m) => m.y))
    );
    expect(Math.min(...out["0.5"].min.map((m) => m.y))).toBeLessThan(
      Math.min(...out["0.9"].min.map((m) => m.y))
    );
  ...
  it("knowledge function exp ( - r alpha ) for alpha 0.1, 0.01, 0.001", () => {
    ...
    const rEq = (pC: number, alpha: number): number =>
      -Math.log(1 - pC) / alpha;
