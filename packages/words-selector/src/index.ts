...
export class WordSelector {
  ...
  constructor(options?: WordSelectorOptions) {
    if (options) {
      ...
      if (
        "answersCorrectness" in options &&
        Array.isArray(options.answersCorrectness)
      ) {
  ...
  public getRange(r: number): Range | undefined {
    return this.history_ranges.find(
      (range) => range.from <= r && range.to >= r
    );
  }

  public nearest(r: number, range: Range): number {
    return range.to - r >= r - range.from && range.from > 1
      ? range.from - 1
      : range.to + 1;
  }
